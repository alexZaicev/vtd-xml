...
/**
 * The VTD Navigator allows one to navigate XML document represented in VTD records and Location
 * caches. There is one and only one cursor that you can navigate to any part of the tree. If a
 * method operating on a node doesn't accept the node as input, by default it refers to the cursor
 * element. The hierarchy consists entirely of elements.
 */
public class VTDNav {
  ...
  protected static final long MASK_TOKEN_FULL_LEN = 0x000fffff00000000L;
  protected static final long MASK_TOKEN_PRE_LEN = 0x000ff80000000000L;
  protected static final long MASK_TOKEN_QN_LEN = 0x000007ff00000000L;
  protected long MASK_TOKEN_OFFSET = 0x000000003fffffffL;
  protected static final long MASK_TOKEN_TYPE = 0xf000000000000000L;
  protected static final long MASK_TOKEN_DEPTH = 0x0ff0000000000000L;

  // tri-state variable for namespace lookup
  protected static final long MASK_TOKEN_NS_MARK = 0x00000000c0000000L;
   ...
   * @param offset int starting offset of the document(in byte)
  ...
  protected VTDNav(
      ...
      FastLongBuffer l2,
      FastIntBuffer l3,
      int offset, // start offset of the starting offset(in byte)
    ...
    if (ns == false) MASK_TOKEN_OFFSET = 0x000000007fffffffL; // this allows xml size to
    // be 2GB
    ...
    contextStack = new ContextBuffer(10, this.depth + 9);
    contextStack2 = new ContextBuffer(10, this.depth + 9);
    stackTemp = new int[this.depth + 9];
  ...
  /**
   * Return the attribute count of the element at the cursor position. when ns is false, attr_ns
   * tokens are considered attributes; otherwise, ns tokens are not considered attributes
   *
   * @return int
   */
   ...
   * @exception com.ximpleware.NavException The exception if the underlying byte content contains
   *     various errors. Notice that we are being conservative in making little assumption on the
   *     correctness of underlying byte content. This is because the VTD etc can be generated by
   *     another machine from a load-balancer.
   * @exception IllegalArguementException if an is null
   */
  public int getAttrVal(String an) throws NavException {
    // int size = vtdBuffer.size();
    if (context[0] == -1 /*|| atTerminal*/) return -1;
    int index = (context[0] != 0) ? context[context[0]] + 1 : rootIndex + 1;

    int type;
    if (index < vtdSize) type = getTokenType(index);
    else return -1;
    if (ns == false) {
      while ((type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)) {
        ...
        index += 2;
        if (index >= vtdSize) break;
        type = getTokenType(index);
      }
    } else {
      while ((type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)) {
        if (type == TOKEN_ATTR_NAME
            && matchRawTokenString(index, an)) { // ns node visible only ns is disabled
          return index + 1;
        }
        index += 2;
        if (index >= vtdSize) break;
  ...
  /**
   * Get the token index of the attribute value of given URL and local name. If ns is not enabled,
   * the lookup will return -1, indicating a no-found. Also namespace nodes are invisible using this
   * method. One can't use * to indicate any name space because * is ambiguous!!
   *
  ...
  public int getAttrValNS(String URL, String ln) throws NavException {
    if (!ns || context[0] == -1 /*|| atTerminal*/) return -1;
    if (URL != null && URL.length() == 0) URL = null;
    if (URL == null) return getAttrVal(ln);
    int size = vtdBuffer.size;
    int index = (context[0] != 0) ? context[context[0]] + 1 : rootIndex + 1;
  ...
  private long handle_utf16le(int offset) throws NavException {
    ...
    int val,
        temp = (XMLDoc.byteAt((offset << 1) + 1) & 0xff) << 8 | (XMLDoc.byteAt(offset << 1) & 0xff);
  ...
  private long handle_utf16be(int offset) throws NavException {
    ...
    int temp =
        ((XMLDoc.byteAt(offset << 1) & 0xff) << 8) | (XMLDoc.byteAt((offset << 1) + 1) & 0xff);
    if ((temp < 0xd800) || (temp > 0xdfff)) { // not a high surrogate
      ...
      if (temp > 0xdbff) throw new NavException("UTF 16 BE encoding error: should never happen");
      val = temp;
      temp =
          ((XMLDoc.byteAt((offset << 1) + 2) & 0xff) << 8)
  ...
  /**
   * This method decodes the underlying byte array into corresponding UCS2 char representation . It
   * doesn't resolves built-in entity and character references. Length will never be zero Creation
   * date: (11/21/03 6:26:17 PM)
   *
   * @return int
   * @exception com.ximpleware.NavException The exception is thrown if the underlying byte content
   *     contains various errors. Notice that we are being conservative in making little assumption
   *     on the correctness of underlying byte content. This is because the VTD can be generated by
   *     another machine, e.g. from a load-balancer.
   */
        ...
        if (ch == 'x') {
          while (true) {
            ...
            if (ch >= '0' && ch <= '9') {
              ...
              inc++;
              break;
            } else throw new NavException("Illegal char in a char reference");
          }
        } else {
          while (true) {
            ...
            if (ch >= '0' && ch <= '9') {
              ...
              break;
            } else throw new NavException("Illegal char in char reference");
          }
        }
        break;
      ...
      case 'a':
        ...
        if (ch == 'm') {
          if (getCharUnit(offset + 1) == 'p' && getCharUnit(offset + 2) == ';') {
            inc = 5;
            val = '&';
          } else throw new NavException("illegal builtin reference");
        } else if (ch == 'p') {
  ...
  public void dumpXML(OutputStream os) throws java.io.IOException {
    os.write(this.XMLDoc.getBytes(), this.offset, this.length);
  }
    ...
    return (encoding <= 2)
        ...
        : (encoding <= FORMAT_WIN_1258)
            ...
            : (encoding == FORMAT_UTF_16BE)
                ? (((int) XMLDoc.byteAt(offset << 1)) << 8 | XMLDoc.byteAt((offset << 1) + 1))
                : (((int) XMLDoc.byteAt((offset << 1) + 1)) << 8 | XMLDoc.byteAt(offset << 1));
  }
  /**
  ...
  /**
   * getElementFragmentNS returns a ns aware version of the element fragment encapsulated in an
   * ElementFragment object
   *
  ...
  /**
   * Return the byte offset and length of up to i sibling fragments. If there is a i+1 sibling
   * element, the cursor element would move to it; otherwise, there is no cursor movement. If the
   * cursor isn't positioned at an element (due to XPath evaluation), then -1 will be returned
   *
   * @param i number of silbing elements including the cursor element
   * @return a long encoding byte offset (bit 31 to bit 0), length (bit 62 to bit 32) of those
   *     fragments
   * @throws NavException
   */
  public long getSiblingElementFragments(int i) throws NavException {
    if (i <= 0) throw new IllegalArgumentException(" # of sibling can be less or equal to 0");
    // get starting char offset
    if (atTerminal == true) return -1L;
    // so is the char offset
    int so = getTokenOffset(getCurrentIndex()) - 1;
    // char offset to byte offset conversion
    if (encoding >= FORMAT_UTF_16BE) so = so << 1;
    ...
    } else bm.setCursorPosition();
    return (((long) len) << 32) | so;
  }

  /**
   * Get the starting offset and length of an element encoded in a long, upper 32 bits is length;
   * lower 32 bits is offset Unit is in byte. Creation date: (3/15/04 1:47:55 PM)
   */
  public long getElementFragment() throws NavException {
    ...
    if (depth == -1) {
      ...
      if (i == 0) return ((long) length) << 32 | offset;
      else return ((long) (length - 32)) | 32;
    }
    ...
    if (toElement(NEXT_SIBLING)) {
      ...
      if (encoding <= FORMAT_WIN_1258) return ((long) length) << 32 | so;
      else return ((long) length) << 33 | (so << 1);
    }
    ...
    if (depth == 0) {
      ...
      else so2 = getTokenOffset(temp + 1);
      while (getCharUnit(so2) != '>') {
        so2--;
      }
      length = so2 - so + 1;
  ...
  /**
   * Return the charater (not byte) offset after head (the ending bracket of the starting tag, not
   * including an empty element, in which case 0xffffffff 00000000 | len is returned)
   *
   * @return
   */
  public final long getOffsetAfterHead() {
    ...
    int j = i + 1;
    while (j < vtdSize
        && (getTokenType(j) == VTDNav.TOKEN_ATTR_NAME || getTokenType(j) == VTDNav.TOKEN_ATTR_NS)) {
      j += 2;
    }
    ...
    if (i + 1 == j) {
      ...
      offset =
          getTokenOffset(j - 1)
              + ((ns == false) ? getTokenLength(j - 1) : (getTokenLength(j - 1) & 0xff))
              + 1;
    }
    ...
    if (getCharUnit(offset - 1) == '/') return 0xffffffff00000000L | (offset);
    else return offset + 1;
  }

  protected final long getOffsetBeforeTail() throws NavException {
    long l = getElementFragment();
    ...
    offset += len;
    if (getCharUnit(offset - 2) == '/') return 0xffffffff00000000L | offset - 1;
    else {
      offset -= 2;
      while (getCharUnit(offset) != '/') {
        offset--;
  ...
  /**
   * This method returns of the token index of the type character data or CDATA. Notice that it is
   * intended to support data orient XML (not mixed-content XML). return the index of the text
   * token, or -1 if none exists.
   *
   * @return int
   */
  public int getText() {
    ...
    int depth = getCurrentDepth();
    int type;
    if (index < vtdSize || !atTerminal) type = getTokenType(index);
    else return -1;

    while (true) {
      if (type == TOKEN_CHARACTER_DATA || type == TOKEN_CDATA_VAL) {
        if (depth == getTokenDepth(index)) return index;
        else return -1;
  ...
  /**
   * Get the token length at the given index value please refer to VTD spec for more details Length
   * is in terms of the UTF char unit For prefixed tokens, it is the qualified name length. When ns
   * is not enabled, return the full name length for attribute name and element name When ns is
   * enabled, return an int with upper 16 bit for prefix length, lower 16 bit for qname length
   *
   * @return int
   * @param index int
   */
    ...
    switch (type) {
      ...
      case TOKEN_COMMENT: // make sure this is total length
        ...
        do {
          ...
          temp =
              getTokenOffset(index) + (int) ((vtdBuffer.longAt(index) & MASK_TOKEN_FULL_LEN) >> 32);
          index++;
        } while (index < vtdSize
            && depth == getTokenDepth(index)
            && type == getTokenType(index)
  ...
  public final int getTokenType(int index) {
    long val = vtdBuffer.longAt(index);
    val = (val & MASK_TOKEN_TYPE) >> 60;
    return (int) val & 0xf;
  }
  /**
   * Test whether current element has an attribute with the matching name. "*" will match any
   * attribute name, therefore is a test whether there is any attribute at all if namespace is
   * disabled, this function will not distinguish between ns declaration and attribute otherwise, ns
   * tokens are invisible Creation date: (11/16/03 5:50:26 PM)
   *
   ...
   * @exception IllegalArguementException if an is null
   */
  public final boolean hasAttr(String an) throws NavException {
    return getAttrVal(an) != -1;
  }
  /**
   * Test whether the current element has an attribute with matching namespace URL and localname. If
   * ns is false, return false immediately
   *
   * @return boolean
   ...
   * @exception com.ximpleware.NavException The exception if the underlying byte content contains
   *     various errors. Notice that we are being conservative in making little assumption on the
   *     correctness of underlying byte content. This is because the VTD can be generated by another
   *     machine such as a load-balancer.
   * @exception IllegalArguementException if ln is null
   */
  public final boolean hasAttrNS(String URL, String ln) throws NavException {
        ...
        case TOKEN_STARTING_TAG:
          ...
          if (index != a[depth]) {
            if (en.equals("*") || matchRawTokenString(index, en)) {
              context[0] = depth;
              if (depth > 0) context[depth] = index;
              if (depth < maxLCDepthPlusOne) resolveLC();
              atTerminal = false;
              return true;
            } else {
              context[depth] = index;
  ...
  /**
   * This method is similar to getElementByName in DOM except it doesn't return the nodeset, instead
   * it iterates over those nodes. Notice that this method is called by the "iterate" method in the
   * Autopilot class. "*" will match any element Creation date: (12/2/03 2:31:20 PM)
   *
   * @return boolean
   * @param dp int (The depth of the starting position before iterating)
   * @param en java.lang.String
   * @exception com.ximpleware.NavException The exception is signaled if the underlying byte content
   *     contains various errors. Notice that we are being conservative in making little assumption
   *     on the correctness of underlying byte content. This is because VTD records can be generated
   *     by another machine from a load-balancer. null element name allowed represent node()in
   *     XPath;
   */
    ...
    while (index < vtdSize) {
      ...
      if (tokenType == VTDNav.TOKEN_STARTING_TAG || tokenType == VTDNav.TOKEN_DOCUMENT) {
        ...
        if (depth > dp) {
          ...
          if (depth > 0) context[depth] = index;
          if (special || matchElement(en)) {
            if (dp < maxLCDepthPlusOne) resolveLC();
            return true;
          }
        } else {
          return false;
  ...
  /**
   * This method is similar to getElementByName in DOM except it doesn't return the nodeset, instead
   * it iterates over those nodes . When URL is "*" it will match any namespace if ns is false,
   ...
   * @exception com.ximpleware.NavException The exception if the underlying byte content contains
   *     various errors. Notice that we are being conservative in making little assumption on the
   *     correctness of underlying byte content. This is because VTD records can be generated by
   *     another machine from a load-balancer..
   * @exception IllegalArguementException if ln is null example
   *     <p>int depth = nv.getCurrentDepth() while(iterateNS(depth, "www.url.com","node_name")){
   *     push(); // store the current position //move position safely pop(); // load the position }
   */
   ...
   */
  public final boolean matchElement(String en) throws NavException {

    if (en.equals("*") && context[0] != -1) return true;
    if (context[0] == -1) return false;
    return matchRawTokenString((context[0] == 0) ? rootIndex : context[context[0]], en);
  }
  /**
   * Test whether the current element matches the given namespace URL and localname. URL, when set
   * to "*", matches any namespace (including null), when set to null, defines a "always-no-match"
   * ln is the localname that, when set to *, matches any localname
   *
   * @return boolean
   * @param URL java.lang.String
   * @param ln java.lang.String
   * @exception com.ximpleware.NavException When there is any encoding conversion error or unknown
   *     entity.
   */
  public final boolean matchElementNS(String URL, String ln) throws NavException {
    if (context[0] == -1) return false;
    ...
    if (ln.equals("*")
      ...
      if (preLen == 3
          ...
          && URL.equals("http://www.w3.org/XML/1998/namespace")) return true;
    }
    return false;
  }

  private final boolean matchRawTokenString(int offset, int len, String s) throws NavException {
    return compareRawTokenString(offset, len, s) == 0;
  }

  protected final int compareTokenString(int offset, int len, String s) throws NavException {
    int i, l;
    long l1;
    ...
    for (i = 0; i < l && offset < endOffset; i++) {
      ...
      int i1 = s.charAt(i);
      if (i1 < (int) l1) return 1;
      if (i1 > (int) l1) return -1;
      offset += (int) (l1 >> 32);
    }

    if (i == l && offset < endOffset) return 1;
    if (i < l && offset == endOffset) return -1;
    return 0;
  }

  /**
   * <em>New in 2.0</em> This method compares two VTD tokens of VTDNav objects The behavior of this
   * method is like compare the strings corresponds to i1 and i2, meaning for text or attribute val,
   * entities will be converted into the corresponding char, return 0 if two tokens are the
   * identical when converted to Unicode String using toString() respectively
   *
   * @param i1
   * @param vn2
   * @param i2
  ...
  public final int compareTokens(int i1, VTDNav vn2, int i2) throws NavException {
    ...
    int len1 =
        (t1 == TOKEN_STARTING_TAG || t1 == TOKEN_ATTR_NAME || t1 == TOKEN_ATTR_NS)
            ? getTokenLength(i1) & 0xffff
            : getTokenLength(i1);
    int len2 =
        (t2 == TOKEN_STARTING_TAG || t2 == TOKEN_ATTR_NAME || t2 == TOKEN_ATTR_NS)
    ...
    if (offset1 == endOffset1 && offset2 < endOffset2) return -1;
    else if (offset1 < endOffset1 && offset2 == endOffset2) return 1;
    else return 0;
  }
  /**
   * Lexicographically compare a string against a token with given offset and len, entities doesn't
   * get resolved.
   *
   ...
   *     machine such as a load-balancer.
   */
  protected final int compareRawTokenString(int offset, int len, String s) throws NavException {
    int i, l;
    long l1;
    // this.currentOffset = offset;
    int endOffset = offset + len;
  ...
  /**
   * <em>New in 2.0</em> Compare the string against the token at the given index value. When a token
   * is an attribute name or starting tag, qualified name is what gets compared against This method
   * has to take care of the underlying encoding conversion but it <em> doesn't </em> resolve entity
   * reference in the underlying document The behavior is the same as calling toRawString on index,
   * then compare to s
   *
   * @param index
   * @param s
   ...
   */
  public final int compareRawTokenString(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
  ...
  /**
   * Match the string against the token at the given index value. When a token is an attribute name
   * or starting tag, qualified name is what gets matched against This method has to take care of
   * the underlying encoding conversion but it <em> doesn't </em> resolve entity reference in the
   * underlying document
   *
   * @return boolean
   * @param index int (index into the VTD token buffer)
   * @param s java.lang.String
   * @exception com.ximpleware.NavException When if the underlying byte content contains various
   *     errors. Notice that we are being conservative in making little assumption on the
   *     correctness of underlying byte content. This is because the VTD can be generated by another
   *     machine such as a load-balancer.
   */
  public final boolean matchRawTokenString(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
  ...
  /**
   * <em>New in 2.0</em> Compare the string against the token at the given index value. When a token
   * is an attribute name or starting tag, qualified name is what gets matched against This method
   * has to take care of the underlying encoding conversion as well as entity reference comparison
   *
   * @param index
   * @param s
   * @return int
   * @throws NavException
   */
  public final int compareTokenString(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
    ...
    else return compareRawTokenString(getTokenOffset(index), len, s);
  }
  /**
   * Match the string against the token at the given index value. When a token is an attribute name
   * or starting tag, qualified name is what gets matched against This method has to take care of
   * the underlying encoding conversion as well as entity reference comparison
   *
   * @return boolean
   * @param index int
   * @param s java.lang.String
   ...
   *     machine such as a load-balancer.
   */
  public final boolean matchTokenString(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
  ...
  /**
   * Match the string against the token at the given index value. The token will be interpreted as
   * if it is normalized (i.e. all white space char (\r\n\a ) is replaced by a white space, char
   * entities and entity references will be replaced by their correspondin char see xml 1.0 spec
   * interpretation of attribute value normalization)
   *
   * @param index
   * @param s
   * @return
   * @throws NavException
   */
  protected final boolean matchNormalizedTokenString2(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
  ...
  protected final int compareNormalizedTokenString2(int offset, int len, String s)
      throws NavException {
    int i, l, temp;
    long l1, l2;
    // boolean b = false;
  ...
  /**
   * Match a string against a "non-extractive" token represented by a long (upper 32 len, lower 32
   * offset).
   *
   * @return boolean
   * @param l long
   * @param s java.lang.String
   * @exception com.ximpleware.NavException When the underlying byte content contains various
   *     errors. Notice that we are being conservative in making little assumption on the
   *     correctness of underlying byte content. This is because the VTD can be generated by another
   *     machine such as a load-balancer.
   */
  ...
  /**
   * overWrite is introduced in version 2.0 that allows you to directly overwrite the XML content if
   * the token is long enough If the operation is successful, the new content along with whitespaces
   * will fill the available token space, and there will be no need to regenerate the VTD and LCs
   * !!! <em> The current version (2.0) only allows overwrites on attribute value, character data,
   * and CDATA</em>
   *
   * <p>Consider the XML below: <a>good </a> After overwriting the token "good" with "bad," the new
   * XML looks like: <a>bad </a> as you can see, "goo" is replaced with "bad"
   * character-by-character, and the remaining "d" is replace with a white space
   *
   * @param index
   ...
   */
  public final boolean overWrite(int index, byte[] ba) {
    return overWrite(index, ba, 0, ba.length);
  }

  /**
   * overWrite is introduced in version 2.0 that allows you to directly overwrite the XML content if
   * the token is long enough If the operation is successful, white spaces will be used to fill the
   * available token space, and there will be no need to regenerate the VTD and LCs <em> The current
   * version (2.0) only allows overwrites on attribute value, character data, and CDATA</em>
   *
   * <p>Consider the XML below: <a>good </a> After overwriting the token "good" with "bad," the new
   * XML looks like: <a>bad </a> as you can see, "goo" is replaced with "bad", and the remaining "d"
   * is replace with a white space
   *
  ...
  public boolean overWrite(int index, byte[] ba, int offset, int len) {
    if (ba == null || index >= this.vtdSize || offset < 0 || offset + len > ba.length)
  ...
  public double parseDouble(int index) throws NavException {
    ...
    if (ch == 'E' || ch == 'e') {
      ...
      if (cur == offset) return Double.NaN;
    ...
    if (right != 0) v += ((double) right) / (double) scale;

    if (exp != 0) v = (expneg) ? v / (Math.pow(10, exp)) : v * Math.pow(10, exp);

    return ((neg) ? (-v) : v);
  }

  /**
   * Convert a vtd token into a float. we assume token type to be attr val or character data
  ...
  public float parseFloat(int index) throws NavException {
    ...
    if (ch == 'E' || ch == 'e') {
      ...
      if (cur == offset) return Float.NaN;
      // found a invalid number like 1.23E
    ...
    // anything left must be space
    while (offset <= end) {
      if (!isWS(ch)) throw new NavException(toString(index));
    ...
    if (exp != 0) v = v * Math.pow(10, exp);

    float f = (float) v;

    // try to handle overflow/underflow
    if (v >= (double) Float.MAX_VALUE) f = Float.MAX_VALUE;
    else if (v <= (double) Float.MIN_VALUE) f = Float.MIN_VALUE;
    if (neg) f = -f;
    return f;
  }
  /**
   * Convert a vtd token into an int. This method will automatically strip off the leading and
   * trailing we assume token type to be attr val or character data zero, unlike
   * Integer.parseInt(int index)
   *
   * <p>Creation date: (12/8/03 2:32:22 PM)
   *
   * @return int
   * @param index int
   * @exception com.ximpleware.NavException The exception if the underlying byte content contains
  ...
  /**
   * Convert a vtd token into an int, with the given radix. we assume token type to be attr val or
   * character data the first char can be either '+' or '-' Creation date: (12/16/03 1:21:20 PM)
   *
   * @return int
   * @param index int
   * @param radix int
  ...
  protected int parseInt(int index, int radix) throws NavException {
    ...
    if (result > Integer.MAX_VALUE) throw new NumberFormatException("Overflow: " + toString(index));
    ...
    if (offset == (endOffset + 1)) return (int) ((neg) ? (-result) : result);
    else throw new NumberFormatException(toString(index));
  }
  /**
   * Convert a vtd token into a long. we assume token type to be attr val or character data Creation
   * date: (12/8/03 2:32:59 PM)
   *
   * @return long
   * @param index int
   * @exception com.ximpleware.NavException The exception if the underlying byte content contains
  ...
  /**
   * Convert a vtd token into a long, with the given radix. the first char can be either '+' or '-',
   * leading and trailing will be stripped we assume token type to be attr val or character data
  ...
  protected long parseLong(int index, int radix) throws NavException {
    ...
    if (result > Long.MAX_VALUE) throw new NumberFormatException("Overflow: " + toString(index));
    ...
    if (offset == (endOffset + 1)) return (long) ((neg) ? (-result) : result);
    else throw new NumberFormatException(toString(index));
  }

  /**
   * Load the context info from ContextBuffer. Info saved including LC and current state of the
   * context
   *
   * @return boolean
   */
  public boolean pop() {
    ...
    l2lower = stackTemp[depth + 3];
    l2upper = stackTemp[depth + 4];
    l3lower = stackTemp[depth + 5];
    l3upper = stackTemp[depth + 6];
    atTerminal = (stackTemp[depth + 7] == 1);
    LN = stackTemp[depth + 8];
    return true;
  }
  /**
  ...
  /**
   * Store the context info into the ContextBuffer. Info saved including LC and current state of the
   * context Creation date: (11/16/03 7:00:27 PM)
   */
  public void push() {
    ...
    stackTemp[depth + 3] = l2lower;
    stackTemp[depth + 4] = l2upper;
    stackTemp[depth + 5] = l3lower;
    stackTemp[depth + 6] = l3upper;
    if (atTerminal) stackTemp[depth + 7] = 1;
    else stackTemp[depth + 7] = 0;
    stackTemp[depth + 8] = LN;
  ...
  /**
   * Sync up the current context with location cache. This operation includes finding out l1index,
   * l2index, l3index and restores upper and lower bound info To improve efficieny this method
   * employs some heuristic search algorithm. The result is that it is quite close to direct access.
  ...
  protected void resolveLC() {
    ...
    resolveLC_l1();
    if (context[0] == 1) return;
    resolveLC_l2();
    if (context[0] == 2) return;
    resolveLC_l3();
  }
  ...
  protected void resolveLC_l2() {
    ...
    if (l2index < 0 || l2index >= l2Buffer.size || context[2] != l2Buffer.upper32At(l2index)) {
      ...
      if (l2index >= l2Buffer.size || l2index < 0) l2index = l2lower;
      if (l2index + 1 < l2Buffer.size && context[2] == l2Buffer.upper32At(l2index + 1))
                ...
                + (int)
                    ((l2upper - l2lower)
                        * ((float) context[2] - l2Buffer.upper32At(l2lower))
                        / (l2Buffer.upper32At(l2upper) - l2Buffer.upper32At(l2lower)));
        if (l2Buffer.upper32At(init_guess) > context[2]) {
          while (context[2] != l2Buffer.upper32At(init_guess)) init_guess--;
        ...
        while (context[2] != l2Buffer.upper32At(l2index)) l2index++;
      }
    }
  }
  ...
  protected void resolveLC_l3() {
    ...
    if (l3index < 0 || l3index >= l3Buffer.size || context[3] != l3Buffer.intAt(l3index)) {
      if (l3index >= l3Buffer.size || l3index < 0) l3index = l3lower;
                ...
                + (int)
                    ((l3upper - l3lower)
                        * ((float) (context[3] - l3Buffer.intAt(l3lower))
                            / (l3Buffer.intAt(l3upper) - l3Buffer.intAt(l3lower))));
        if (l3Buffer.intAt(init_guess) > context[3]) {
          while (context[3] != l3Buffer.intAt(init_guess)) init_guess--;
  ...
  /**
   * Test whether the URL is defined in the scope. Null is allowed to indicate the name space is
   * undefined. Creation date: (11/16/03 7:54:01 PM)
   *
   * @param URL java.lang.String
   * @exception com.ximpleware.NavException When there is any encoding conversion error or unknown
   *     entity.
  ...
  protected final int lookupNS() throws NavException {
    if (context[0] == -1) throw new NavException("Can't lookup NS for document node");
    int i = getTokenLength((context[0] != 0) ? context[context[0]] : rootIndex);
    int offset = getTokenOffset((context[0] != 0) ? context[context[0]] : rootIndex);
    int preLen = (i >> 16) & 0xffff;
  ...
  /**
   * This function returns the VTD record index of the namespace that matches the prefix of cursor
   * element
   *
   * @param URL
   * @return int
  ...
  private boolean resolveNS(String URL, int offset, int len) throws NavException {
    ...
    switch (result) {
      ...
      default:
        if (URL == null) {
          if (getTokenLength(result) == 0) return true;
          return false;
        } else {
  ...
  /**
   * A generic navigation method. Move the cursor to the element according to the direction
   * constants If no such element, no position change and return false. Creation date: (12/2/03
   * 1:43:50 PM) Legal direction constants are
   *
   * <pre>
   *    			ROOT               0
   * </pre>
  ...
  /**
   * A generic navigation method. Move the cursor to the element according to the direction
   * constants and the element name If no such element, no position change and return false. "*"
   * matches any element Creation date: (12/2/03 1:43:50 PM) Legal direction constants are <br>
   *
   * <pre>
   * 		ROOT            0
   * </pre>
   ...
   * @param direction int
   * @param en String
   * @exception com.ximpleware.NavException When direction value is illegal. Or there are errors in
  ...
  public boolean toElement(int direction, String en) throws NavException {
    ...
    int val = 0;
    boolean b = false;
    if (en == null) throw new IllegalArgumentException(" Element name can't be null ");
    if (en.equals("*")) return toElement(direction);
    switch (direction) {
      case ROOT:
      ...
      case FIRST_CHILD:
        if (atTerminal) return false;
        if (toElement(FIRST_CHILD) == false) return false;
        // check current element name
        if (matchElement(en) == false) {
          if (toElement(NEXT_SIBLING, en) == true) return true;
          else {
            // toParentElement();
      ...
      case LAST_CHILD:
        if (atTerminal) return false;
        if (toElement(LAST_CHILD) == false) return false;
        if (matchElement(en) == false) {
          if (toElement(PREV_SIBLING, en) == true) return true;
          else {
            // context[context[0]] = 0xffffffff;
            context[0]--;
            // toParentElement();
  ...
  /**
   * A generic navigation method with namespace support. Move the cursor to the element according to
   * the direction constants and the prefix and local names If no such element, no position change
   * and return false. URL * matches any namespace, including undefined namespaces a null URL means
   * hte namespace prefix is undefined for the element ln * matches any localname Creation date:
   ...
   *
   * <br>
   * for ROOT and PARENT, element name will be ignored. If not ns enabled, return false immediately
    ...
    switch (direction) {
      ...
      case FIRST_CHILD:
        ...
        if (matchElementNS(URL, ln) == false) {
          if (toElementNS(NEXT_SIBLING, URL, ln) == true) return true;
          else {
            // toParentElement();
            // context[context[0]] = 0xffffffff;
            context[0]--;
      ...
      case LAST_CHILD:
        ...
        if (toElement(LAST_CHILD) == false) return false;
        if (matchElementNS(URL, ln) == false) {
          if (toElementNS(PREV_SIBLING, URL, ln) == true) return true;
          else {
            // context[context[0]] = 0xffffffff;
            context[0]--;
            // toParentElement();
  ...
  /**
   * (New since version 2.9) Shallow Normalization follows the rules below to normalize a token into
   * a string *#xD#xA gets converted to #xA *For a character reference, append the referenced
   * character to the normalized value. *For an entity reference, recursively apply step 3 of this
   * algorithm to the replacement text of the entity. *For a white space character (#x20, #xD, #xA,
   * #x9), append a space character (#x20) to the normalized value. *For another character, append
  ...
  public String toNormalizedString2(int index) throws NavException {
    int type = getTokenType(index);
    if (type != TOKEN_CHARACTER_DATA && type != TOKEN_ATTR_VAL) return toRawString(index);
    long l;
    int len;
  ...
  /**
   * This method normalizes a token into a string value of character data and attr val in a way that
   * resembles DOM. The leading and trailing white space characters will be stripped. The entity and
   * character references will be resolved Multiple whitespaces char will be collapsed into one.
   * Whitespaces via entities will nonetheless be preserved. Creation date: (12/8/03 1:57:10 PM)
   *
  ...
  public String toNormalizedString(int index) throws NavException {
    ...
    while (offset <= endOffset) {
      ...
      if (isWS(ch) && getCharUnit(offset - 1) != ';') {
        ...
        if (d == false) sb.append((char) ch); // java only supports 16 bit unicode
        else {
          sb.append(' ');
          sb.append((char) ch);
          d = false;
  ...
  public final int getNormalizedStringLength(int index) throws NavException {
    int type = getTokenType(index);
    if (type != TOKEN_CHARACTER_DATA && type != TOKEN_ATTR_VAL) return getRawStringLength(index);
    long l;
    int len, len1 = 0;
    len = getTokenLength(index);
    if (len == 0) return 0;
    ...
    while (offset <= endOffset) {
      ...
      if (isWS(ch) && getCharUnit(offset - 1) != ';') {
        ...
        if (d == false) len1++; // java only supports 16 bit unicode
        else {
          len1 = len1 + 2;
          d = false;
        }
   ...
   * @exception NavException When the encoding has errors
   */
  public final String toRawString(int index) throws NavException {
    int type = getTokenType(index);
    int len;
    if (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
      len = getTokenLength(index) & 0xffff;
    else len = getTokenLength(index);
    int offset = getTokenOffset(index);
    return toRawString(offset, len);
  }

  protected final void toRawString(StringBuilder sb, int index) throws NavException {
    int type = getTokenType(index);
    int len;
    if (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
      len = getTokenLength2(index) & 0xffff;
    else len = getTokenLength2(index);
    int offset = getTokenOffset(index);
    toRawString(offset, len, sb);
  }
  /**
   * Convert a token at the given index to a String, upper case chars get converted into lower case
   ...
   * @throws NavException
   */
  public final String toRawStringLowerCase(int index) throws NavException {
    int type = getTokenType(index);
    int len;
    if (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
      len = getTokenLength(index) & 0xffff;
  ...
  /**
   * Convert a token at the given index to a String, lower case chars get converted into upper case
   * (entities and char references not expanded).
   *
   * @param index
   * @return
   * @throws NavException
   */
  public final String toRawStringUpperCase(int index) throws NavException {
    int type = getTokenType(index);
    int len;
    if (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
      len = getTokenLength(index) & 0xffff;
   ...
   * @throws NavException
   */
  public final String toRawString(int os, int len) throws NavException {
    StringBuffer sb = new StringBuffer(len);
    int offset = os;
  ...
  protected final void toRawString(int os, int len, StringBuffer sb) throws NavException {
    int offset = os;
    int endOffset = os + len;
    long l;
    while (offset < endOffset) {
  ...
  protected final void toRawString(int os, int len, StringBuilder sb) throws NavException {
    int offset = os;
    int endOffset = os + len;
    long l;
    while (offset < endOffset) {
  ...
  protected final void toString(int os, int len, StringBuilder sb) throws NavException {
    int offset = os;
    int endOffset = os + len;
    long l;
    while (offset < endOffset) {
  ...
  protected final void toStringUpperCase(int os, int len, StringBuilder sb) throws NavException {
    ...
    while (offset < endOffset) {
      l = getCharResolved(offset);
      offset += (int) (l >> 32);
      if ((int) l > 96 && (int) l < 123) sb.append((char) (l - 32));
      else sb.append((char) l);
    }
    // return sb.toString();
  }

  protected final void toStringLowerCase(int os, int len, StringBuilder sb) throws NavException {
    // StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
    while (offset < endOffset) {
      l = getCharResolved(offset);
      offset += (int) (l >> 32);
      if ((int) l > 64 && (int) l < 91) sb.append((char) (l + 32));
      else sb.append((char) l);
    }
    // return sb.toString();
  }
  ...
  /**
   * getStringLength return the string length of a token as if the token is converted into a string
   * (entity resolved for character data and attr val)
   *
   * @param index
   ...
   * character data and attr val). An attribute name or an element name will get the UCS2 string of
   * qualified name Creation date: (11/16/03 7:27:19 PM)
   *
   * @return java.lang.String
   * @param index
   * @exception NavException
  ...
  /**
   * Convert a token at the given index to a String and any lower case character will be converted
   * to upper case, (entities and char references resolved character data and attr val).
   *
   * @param index
   * @return
   * @throws NavException
  ...
  public String toStringUpperCase(int index) throws NavException {
    int type = getTokenType(index);
    if (type != TOKEN_CHARACTER_DATA && type != TOKEN_ATTR_VAL) return toRawStringUpperCase(index);
    int len;
    len = getTokenLength(index);
  ...
  /**
   * Convert a token at the given index to a String and any upper case character will be converted
   * to lower case, (entities and char references resolved for character data and attr val).
   *
   * @param index
   * @return
   * @throws NavException
  ...
  public String toStringLowerCase(int index) throws NavException {
    int type = getTokenType(index);
    if (type != TOKEN_CHARACTER_DATA && type != TOKEN_ATTR_VAL) return toRawStringLowerCase(index);
    int len;
    len = getTokenLength(index);
  ...
  /**
   * Convert the byte content segment (in terms of offset and length) to String (entities are
   * resolved)
   *
   ...
   * @throws NavException
   */
  public final String toString(int os, int len) throws NavException {
    StringBuffer sb = new StringBuffer(len);
    int offset = os;
    int endOffset = os + len;
    // if (encoding> FORMAT_WIN_1258){
  ...
  protected final void toString(StringBuilder sb, int index) throws NavException {
    /*int type = getTokenType(index);
    ...
    int offset = getTokenOffset(index);
    if (type != VTDNav.TOKEN_CDATA_VAL) toString(offset, len, sb);
    else toRawString(offset, len, sb);
  }

  protected final void toStringUpperCase(StringBuilder sb, int index) throws NavException {
    /*int type = getTokenType(index);
    if (type!=TOKEN_CHARACTER_DATA &&
    		type!= TOKEN_ATTR_VAL)
    	toRawString(sb, index);*/
    ...
    int offset = getTokenOffset(index);
    if (type != VTDNav.TOKEN_CDATA_VAL) toStringUpperCase(offset, len, sb);
    else toRawStringUpperCase(offset, len, sb);
  }

  protected final void toStringLowerCase(StringBuilder sb, int index) throws NavException {
    /*int type = getTokenType(index);
    if (type!=TOKEN_CHARACTER_DATA &&
    		type!= TOKEN_ATTR_VAL)
    	toRawString(sb, index); */
    ...
    int offset = getTokenOffset(index);
    if (type != VTDNav.TOKEN_CDATA_VAL) toStringLowerCase(offset, len, sb);
    else toRawStringLowerCase(offset, len, sb);
  }

  /**
   * Convert the byte content segment (in terms of offset and length) to String, lower case
   ...
   * @throws NavException
   */
  protected final String toStringUpperCase(int os, int len) throws NavException {
    StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
  ...
  /**
   * Convert the byte content segment (in terms of offset and length) to String, upper case
   ...
   * @throws NavException
   */
  protected final String toStringLowerCase(int os, int len) throws NavException {
    StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
  ...
  protected final String toRawStringLowerCase(int os, int len) throws NavException {
    StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
  ...
  protected final void toRawStringLowerCase(int os, int len, StringBuilder sb) throws NavException {
    // StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
  ...
  protected final String toRawStringUpperCase(int os, int len) throws NavException {
    StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
  ...
  protected final void toRawStringUpperCase(int os, int len, StringBuilder sb) throws NavException {
    // StringBuilder sb = new StringBuilder(len);
    int offset = os;
    int endOffset = os + len;
    long l;
  ...
  /**
   * Set the value of atTerminal This function only gets called in XPath eval when a step calls
   * for @* or child::text()
   *
   * @param b
   */
  ...
  /**
   * Test the start of token content at index i matches the content of s, notice that this is to
   * save the string allocation cost of using String's built-in startsWidth
   *
   * @param i
   * @param s
   * @return
   */
  public final boolean startsWith(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
    ...
    // System.out.println(s);
    for (i = 0; i < l && offset < endOffset; i++) {
      if (b) l1 = getCharResolved(offset);
      else l1 = getChar(offset);
      int i1 = s.charAt(i);
      if (i1 != (int) l1) return false;
      offset += (int) (l1 >> 32);
    }
    return true;
  }
  ...
  /**
   * Test the end of token content at index i matches the content of s, notice that this is to save
   * the string allocation cost of using String's built-in endsWidth
   *
   * @param i
   * @return
   */
  public final boolean endsWith(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
    ...
    l = s.length();
    if (l > len) return false;
    if (b == true) i2 = getStringLength(index);
    else i2 = getRawStringLength(index);
    if (l > i2) return false;
    i2 = i2 - l; // calculate the # of chars to be skipped
    // eat away first several chars
    for (i = 0; i < i2; i++) {
      if (b == true) l1 = getCharResolved(offset);
      else l1 = getChar(offset);
      offset += (int) (l1 >> 32);
    }
    // System.out.println(s);
  ...
  /**
   * Test whether a given token contains s. notie that this function directly operates on the byte
   * content of the token to avoid string creation
   *
   * @param index
   * @param s
   * @return
   * @throws NavException
   */
  public final boolean contains(int index, String s) throws NavException {
    int type = getTokenType(index);
    int len =
        (type == TOKEN_STARTING_TAG || type == TOKEN_ATTR_NAME || type == TOKEN_ATTR_NS)
            ? getTokenLength(index) & 0xffff
    ...
    while (offset < endOffset) {
      gOffset = offset;
      if (endOffset - gOffset < l) return false;
      for (i = 0; i < l && gOffset < endOffset; i++) {
        if (b == true) l1 = getCharResolved(gOffset);
        else l1 = getChar(gOffset);
        int i1 = s.charAt(i);
        gOffset += (int) (l1 >> 32);
        if (i == 0) offset = gOffset;
        if (i1 != (int) l1) break;
      }
      if (i == l) return true;
    }
  ...
  public boolean verifyNodeCorrectness() {
    if (atTerminal) {
      ...
      if (getTokenDepth(LN) != context[0]) return false;
      switch (context[0]) {
        case -1:
          return true;
        case 0:
          ...
          //	return false;
          if (l1Buffer.size != 0) {
            if (l1index >= l1Buffer.size || l1index < 0) return false;
            if (l1index != l1Buffer.size - 1) {

              if (l1Buffer.upper32At(l1index) < LN) return false;
            }
            return true;
          } else return true;
        ...
        case 1:
          if (LN > context[1]) {
            ...
            //	return false;
            if (l1index < 0 || l1index > l1Buffer.size) return false;
            int i1, i2; // l2lower, l2upper and l2index
            i1 = l1Buffer.lower32At(l1index);
            if (i1 != -1) {
              if (i1 != l2lower) return false;
              int tmp = l1index + 1;
              i2 = l2Buffer.size - 1;
              ...
              if (l2upper != i2) return false;
              if (l2index > l2upper || l2index < l2lower) return false;
              if (l2index != l2upper) {
                if (l2Buffer.upper32At(l2index) < LN) return false;
              }
              if (l2index != l2lower) {
                if (l2Buffer.upper32At(l2index - 1) > LN) return false;
              }
            }
            return true;
          } else return false;
        case 2:
          if (LN > context[2] && context[2] > context[1]) {
            ...
            if (i1 != -1) {
              ...
              if (l3upper != i2) return false;

              if (l3index < i1 || l3index > i2) return false;

              if (l3index != l3upper) {
                if (l3Buffer.intAt(l3index) < LN) return false;
              }
              if (l3index != l3lower) {
                if (l3Buffer.intAt(l3index - 1) > LN) return false;
              }
            }
            return true;
          } else return false;
        ...
        default:
          ...
          if (context[context[0]] > LN) return false;
  ...
  public void sampleState(FastIntBuffer fib) {
    ...
    //			context[i] = -1;
    for (int i = 0; i <= context[0]; i++) fib.append(context[i]);
    if (atTerminal) fib.append(LN);
    if (context[0] >= 1) fib.append(l1index);
  ...
  public void writeSeparateIndex(OutputStream os) throws IndexWriteException, IOException {
    IndexHandler.writeSeparateIndex_L3(
        (byte) 2,
        this.encoding,
        this.ns,
        true,
  ...
  public long getIndexsize() {
    int size;
    if ((length & 7) == 0) size = length;
    else size = ((length >> 3) + 1) << 3;
  ...
  /**
   * Duplicate the VTDNav instance with shared XML, VTD and LC buffers This method may be useful for
   * parallel XPath evaluation The node Position is at root element
   *
   * @return a VTDNav instance
   */
  public VTDNav duplicateNav() {
    return new VTDNav(
        rootIndex, encoding, ns, depth - 1, XMLDoc, vtdBuffer, l1Buffer, l2Buffer, l3Buffer, offset,
        length);
  }
  ...
  public VTDNav cloneNav() {
    VTDNav vn =
        new VTDNav(
            rootIndex, encoding, ns, depth - 1, XMLDoc, vtdBuffer, l1Buffer, l2Buffer, l3Buffer,
            offset, length);
  ...
  public final void fillXPathString(FastIntBuffer indexBuffer, FastIntBuffer countBuffer)
      throws NavException {
    int count = 0;
    int index = getCurrentIndex() + 1;
    ...
    while (index < vtdSize) {
      ...
      if (tokenType == VTDNav.TOKEN_CHARACTER_DATA || tokenType == VTDNav.TOKEN_CDATA_VAL) {
        ...
        continue;
        //
      } else if (tokenType == VTDNav.TOKEN_ATTR_NAME || tokenType == VTDNav.TOKEN_ATTR_NS) {
        index = index + 2;
        continue;
      }
      index++;
   ...
   * @throws NavException
   */
  public final String getXPathStringVal(short mode) throws NavException {
  ...
  protected final String getXPathStringVal2(int j, short mode) throws NavException {
    ...
    for (t = 0; t < fib.size; t++) {
      switch (mode) {
        case 0:
          ...
          break;
        case 1:
          toStringUpperCase(sb, fib.intAt(t));
          break;
        case 2:
          toStringLowerCase(sb, fib.intAt(t));
          break;
  ...
  public final boolean XPathStringVal_Contains(int j, String s) throws NavException {
    int tokenType;
    int index = j + 1;
    ...
    while (index < fib.size) {
      ...
      while (offset < endOffset) {
        if (type == VTDNav.TOKEN_CHARACTER_DATA) l = getCharResolved(offset);
        else l = getChar(offset);
        c = (int) l;
        if (c == s.charAt(0) && matchSubString(offset, endOffset, index, type, s)) {
          result = true;
          break loop;
        } else offset += (int) (l >> 32);
      }
      index++;
    }
  ...
  private boolean matchSubString(int os, int eos, int index, int t, String s) throws NavException {
    int offset = os, endOffset = eos, type = t, c;
    long l;
    int i = 0;
    boolean b = false;
    while (offset < endOffset) {
  ...
  public final boolean XPathStringVal_StartsWith(int j, String s) throws NavException {
    int tokenType;
    int index = j + 1;
  ...
  public final boolean XPathStringVal_EndsWith(int j, String s) throws NavException {
    int tokenType;
    int index = j + 1;
  ...
  /**
   * Get content fragment returns a long encoding the offset and length of the byte segment of the
   * content of current element, which is the byte segment between the starting tag and ending tag,
   * -1 is returned if the current element is an empty element
   *
  ...
  /**
   * This method takes a vtd index, and recover its correspondin node position, the index can only
   * be of node type element, document, attribute name, attribute value or character data, or CDATA
   *
   * @param index
   * @throws NavException
   */
  public void recoverNode(int index) throws NavException {
    if (index < 0 || index >= vtdSize) throw new NavException("Invalid VTD index");
    ...
    type == VTDNav.TOKEN_PI_VAL
        ...
        || type == VTDNav.TOKEN_ATTR_VAL) throw new NavException("Token type not yet supported");
    ...
    switch (d) {
      ...
      case 0:
        ...
        if (index != rootIndex) {
          LN = index;
          atTerminal = true;
          if (type > VTDNav.TOKEN_ATTR_NS) sync(0, index);
        } else atTerminal = false;
        return;
    }
    ...
    if (d == 1) {
      if (atTerminal && type > VTDNav.TOKEN_ATTR_NS) sync(1, index);
      return;
    }
    // search LC level 2
    ...
    if (d == 2) {
      // resolveLC();
      if (atTerminal && type > VTDNav.TOKEN_ATTR_NS) sync(2, index);
      return;
    }
    // search LC level 3
    ...
    if (d == 3) {
      // resolveLC();
      if (atTerminal && type > VTDNav.TOKEN_ATTR_NS) sync(3, index);
      return;
    }
    // scan backward
    if (type == VTDNav.TOKEN_STARTING_TAG) {
  ...
  protected final void recoverNode_l1(int index) {
    ...
    if (context[1] == index) {
      ...
      if (i >= l1Buffer.size) i = l1Buffer.size - 1;
  ...
  protected final void recoverNode_l2(int index) {
    ...
    i =
        Math.min(
            l2lower + (int) (((float) (index - t1) / (t2 - t1 + 1)) * (l2upper - l2lower)),
            l2upper);
    ...
    while (l2Buffer.upper32At(i) > index && i > 0) i--;
  ...
  private final void recoverNode_l3(int index) {
    ...
    i =
        Math.min(
            l3lower + (int) (((float) (index - t1) / (t2 - t1 + 1)) * (l3upper - l3lower)),
            l3upper);
    while (i < l3Buffer.size - 1 && l3Buffer.intAt(i) < index) {
      i++;
    }
    while (l3Buffer.intAt(i) > index && i > 0) i--;
  ...
  /**
   * Return the prefix of a token as a string if the token is of the type of starting tag, attribute
   * name, if the the prefix doesn't exist, a null string is returned; otherwise a null string is
   * returned
   *
  ...
  public String getPrefixString(int i) throws NavException {
    if (ns == false) return null;
    int type = getTokenType(i);
    if (type != TOKEN_ATTR_NAME && type != TOKEN_STARTING_TAG) return null;
    int offset = getTokenOffset(i);
    int preLen = getTokenLength(i) >> 16;
    if (preLen != 0) return toRawString(offset, preLen);
              ...
              switch (tokenType) {
                case TOKEN_STARTING_TAG:
                  ...
                  if (depth <= context[0]) {
                    ...
                    context[context[0]] = index;
                    return true;
                  } else throw new NavException("impossible condition");
        ...
        switch (context[0]) {
          ...
          case 0:
            if (atTerminal) {
              ...
              if (tokenType == VTDNav.TOKEN_ATTR_NAME) return false;
              // index++;
              if (l1Buffer.size != 0) {
                if (index < l1Buffer.upper32At(l1index)) {
                  index++;
                  if (tokenType == TOKEN_PI_NAME) index++;
                  if (index <= l1Buffer.upper32At(l1index)) {
                    if (index == l1Buffer.upper32At(l1index)) {
                      atTerminal = false;
                      context[0] = 1;
                    ...
                    < l1Buffer.size - 1) { // whether l1index is the last entry in l1 buffer
                  l1index++;
                  if (tokenType == TOKEN_PI_NAME) index++;
                  if (index <= l1Buffer.upper32At(l1index)) {
                    if (index == l1Buffer.upper32At(l1index)) {
          ...
          case 2:
            if (atTerminal) {
              ...
              if (l2index < l2upper) {
                ...
                // rewind
                while (index > tmp) {
                  if (getTokenDepth(index) < 2) index--;
                  else break;
                }

                if ((
                /*lastEntry!=index &&*/ getTokenDepth(index) == 1)) {
                  LN = index;
                  atTerminal = true;
                  context[0] = 1;
                  return true;
          ...
          case 3:
            if (!atTerminal) {
              ...
              if (l3index < l3upper) {
                ...
                // rewind
                while (index > tmp) {
                  if (getTokenDepth(index) < 3) index--;
                  else break;
                }
                if ((
                /*lastEntry==index &&*/ getTokenDepth(index) == 2)) {
                  LN = index;
                  atTerminal = true;
                  context[0] = 2;
                  return true;
      ...
      case 1:
        if (atTerminal) {
          if (l1Buffer.lower32At(l1index) != -1) {
            ...
            index = LN - 1;
            if (getTokenType(index) == TOKEN_PI_VAL) index--;
            if (index > context[1]) {
              tokenType = getTokenType(index);
          ...
          if (getTokenDepth(index) == 0
            ...
            if (tokenType == TOKEN_PI_VAL) index--;
            context[0] = 0;
            atTerminal = true;
            LN = index;
            return true;
        ...
        if (l1Buffer.lower32At(l1index) != -1) {
          ...
          while (index > tmp) {
            ...
            if (depth < 1) index--;
              ...
              tokenType = getTokenType(index);
              if (tokenType == TOKEN_PI_VAL) LN = index - 1;
              else LN = index;
              atTerminal = true;
              // context[0]=1;
              return true;
  ...
  public final String toNormalizedXPathString(int j) throws NavException {
